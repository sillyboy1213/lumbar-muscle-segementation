...
"""
- 该文件定义了多种用于图像分割模型训练的损失函数，这些损失函数都继承自 PyTorch 的 _Loss 类。
- Dice Loss 相关 :
- `BinarySoftDiceLoss` : 用于二分类的 Soft Dice Loss。
- `SoftDiceLoss` : 用于多分类的 Soft Dice Loss，计算除背景外所有类别的平均 Dice系数。
- `SoftDiceLossV2` : 带权重的多分类 Soft Dice Loss，可以为不同类别设置不同的权重。
- Tversky Loss 相关 : Tversky 指数是 Dice 系数和 Jaccard 系数的一种泛化。
- `BinaryTverskyLoss` (在文件中定义了两次，功能类似): 用于二分类的 Tversky Loss。
- `TverskyLoss` : 用于多分类的 Tversky Loss。
- `TverskyLossV2` : 带权重的多分类 Tversky Loss。
- `FocalTverskyLoss` : Focal Tversky Loss，结合了 Focal Loss 的思想来处理难易样本不平衡问题。
- BCE (Binary Cross Entropy) Loss 相关 :
- `WBCELoss` : 带权重的二元交叉熵损失 (Weighted Binary Cross Entropy Loss)。
- 组合损失函数 :
- `BCE_Dice_Loss` : BCE Loss 和 Soft Dice Loss 的组合。
- `WBCE_Dice_Loss` : 加权 BCE Loss 和 Soft Dice Loss 的组合。
- `MultitaskLoss` : 一个多任务损失示例，结合了 WBCE Loss, Soft Dice Loss, 和 MSE Loss (均方误差损失)。
"""
...
# loss = focal_loss(preds, targets, alpha=0.25, gamma=2.0, reduction='mean')
...
class MultitaskLoss(_Loss):
    ...
    def forward(self, inputs, targets):
        return self.wbce_loss(inputs, targets) + self.dice_loss(inputs, targets) + self.mse_loss(inputs, targets)
